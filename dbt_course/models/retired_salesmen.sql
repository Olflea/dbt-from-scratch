with
  ...
  from last_customers
  left join {{ source('dunder_mifflin', 'employees') }} as employees on employees.employee_id = last_customers.employee_id
  left join employee_status on employee_status.status_id = employees.employee_status_id
  left join {{ source('dunder_mifflin', 'customers') }} as customers on customers.customer_id = last_customers.customer_id
